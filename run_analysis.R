...
trainDataSet<-read.table(trainDataFile,colClasses=c("numeric"), header = FALSE,sep="",fill = TRUE)
...
testDataSet<-read.table(testDataFile,colClasses=c("numeric"), header = FALSE,sep="",fill = TRUE)
...
tidyDataSet<-aggregate(. ~ activity + subject, data = mergedDataSet, FUN = mean, na.rm = TRUE)

## Write the tidy data file to the working directory
write.table(tidyDataSet,file="HCI_TidyDataSet.txt", row.names=FALSE)
